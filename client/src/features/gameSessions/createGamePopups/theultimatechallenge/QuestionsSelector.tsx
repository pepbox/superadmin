    ...
    <div className="w-full max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-4 sm:p-6">
      ...
      <ul className="flex flex-nowrap -mb-px text-sm font-medium text-center">
        ...
        (level) => (
          ...
          <button
            ...
            className={`inline-block px-3 py-2 sm:p-4 ${
            activeTab === level
      ...
      <div className="overflow-x-auto max-h-96">
        ...
        <thead className="bg-gray-50">
          ...
          <th
            scope="col"
            className="w-10 py-3 px-2 sm:px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
          >
            Select
          </th>
          <th
            scope="col"
            className="py-3 px-2 sm:px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
          >
            Question
          </th>
          <th
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          <tr key={question._id}>
            ...
            <td className="py-4 px-2 sm:px-4 text-sm text-gray-900 max-w-xs truncate">
            {question.text.length > 80
              ? question.text.substring(0, 80) + "..."
              : question.text}
            </td>
            <td className="py-4 px-2 sm:px-4 text-sm text-gray-500">
            {question.category}
            </td>
            <td className="py-4 px-2 sm:px-4 text-sm text-gray-500">
            {question.difficulty}
      ...
      <button
        onClick={handleDone}
        className="px-4 sm:px-6 py-2 bg-black text-white rounded-lg hover:bg-gray-800 w-full sm:w-auto"
      >
        Done
      </button>
      </div>
